...
import { DB_URL, CRAWL_MAX_THREADS } from "config";
...
async function createManga(
  ...
  if (parser === null) throw new Error("Unsupported manga source");

  let manga = await parser.parseManga(url);

  manga.chapters.forEach(
    (chap) => (chap.isRead = readChapters.indexOf(chap.link) > -1),
  );
...
async function updateChapters(manga) {
  ...
  for (let i = 0; i < manga.chapters.length; i++) {
    let pos = crawledChapters.findIndex(
      (ch) => ch.link === manga.chapters[i].link,
    );
    if (pos !== -1) crawledChapters[pos] = manga.chapters[i];
  }

  let newChapCount = 0;
  let unreadChapCount = 0;
  crawledChapters.forEach((chap) => {
    if (!chap._id) newChapCount += 1;
    if (!chap.isRead) unreadChapCount += 1;
  });
    ...
    await Promise.all(
      chunks[i].map(async (manga) => {
        try {
          ...
          if (verbose) console.log(`    Update: '${manga.name}'`);
        } catch (e) {
          console.error(`    Fail to update: '${manga.name}'`);
          if (verbose) console.error(e.toString());
        }
      }),
    );
...
function getParser(url) {
  ...
  for (let i = 0; i < parsers.length; i++)
    if (url.match(parsers[i].URLRegex)) return parsers[i];
  return null;
}

async function main() {
  if (process.argv.length < 4) throw "Missing args";

  const action = process.argv[2];
  const url = process.argv[3];
  if (action !== "create" && action !== "update") throw "Invalid action";
